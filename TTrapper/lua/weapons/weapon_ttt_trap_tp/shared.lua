...
SWEP.EntrancePlaced = false

local ghostmdl = Model("models/props_combine/combine_mortar01b.mdl")
...
function SWEP:PrimaryAttack()
	...
	if self.EntrancePlaced then return end
	...
	if tr.HitPos:Distance(self.Owner:GetShootPos()) <= 100 and tr.HitWorld  then
		...
		self.EntrancePlaced = true
		if SERVER then
			local trigger = ents.Create("tp_entrance")
			trigger:SetPos(tr.HitPos)
			trigger:SetAngles(Angle(0,self.Owner:EyeAngles().y + 180,self.Owner:EyeAngles().r))
			trigger.Placer = self.Owner
			trigger:Spawn()
			trigger:Activate()
			//trigger:SetRenderMode( RENDERMODE_TRANSALPHA )
...
function SWEP:SecondaryAttack()
	if self.EntrancePlaced == false then return end
	if SERVER then
		local tr = self.Owner:GetEyeTrace()
		...
		if tr.HitPos:Distance(self.Owner:GetShootPos()) <= 100 and tr.HitWorld  then
			local trigger = ents.Create("tp_exit")
			trigger:SetPos(tr.HitPos)
...
if CLIENT then
	...
	function SWEP:ChangeGhostModel()
		if IsValid(self.Ghost) then
			self.Ghost:SetModel("models/props_junk/sawblade001a.mdl")
   ...
   function SWEP:UpdateGhost(pos, c, a, angle, norm)
      if IsValid(self.Ghost) then
         if self.Ghost:GetPos() != pos then
			...
			if self.ModelType == nil and norm == Vector(0,0,1) then
				self.Ghost:SetAngles(angle + Angle(0,self.Owner:EyeAngles().y + 180,self.Owner:EyeAngles().r) )
			else
	...
	function SWEP:CanPlaceTrap( dist, norm )
		if IsValid(self.Ghost) then
			...
			if norm != Vector(0,0,1) or dist > 100 or tr.HitWorld then
				return false
			end
		end
		return true
